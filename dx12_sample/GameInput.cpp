...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize(HWND hwnd)
    {
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
        ThrowIfFailed(DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&s_DI, nullptr));

        ThrowIfFailed(s_DI->CreateDevice(GUID_SysKeyboard, &s_Keyboard, nullptr));
        ThrowIfFailed(s_Keyboard->SetDataFormat(&c_dfDIKeyboard));
        ThrowIfFailed(s_Keyboard->SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE));
        ...
        ThrowIfFailed(s_Mouse->SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE));
#endif
...
void GameInput::Update(float frameDelta, HWND hwnd)
{
