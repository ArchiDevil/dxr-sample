...
RWTexture2D<float4> transmittanceMap : register(u3); // precomputed transmittance map (read only)
...
void CSMain(uint3 groupId : SV_GroupID)
{
    ...
    float3 ray, mie;
    float3 muMusNu = GetMuMusNu(alt, dhdH, float(groupId.x), float(groupId.y), tableWidth, tableHeight, resNu);
